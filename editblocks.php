...
<div class="container">
  <div class="row">
    <div class="col-sm-8">
        ...
        <button type="button" class="btn btn-warning" onclick="resetWorkspeace()">Reset Workspace</button>
        <?php
        if($user_id == -1){
          echo "  <div class=\"form-group\">";
          echo "    <h3>Bot Name:</h3>";
          echo "      <input style=\"width:200px\" type=\"text\" class=\"form-control\" id=\"botName\" placeholder=\"pizzabot\">";
          echo "    </div>";
          echo "<button type=\"button\" class=\"btn btn-success\" onclick=\"saveXml()\">Save Chatbot</button>";
        } else {
          echo "<button type=\"button\" class=\"btn btn-success\" onclick=\"saveExistXml()\">Save Chatbot</button>";
        }
        ?>
...
<script>
  ...
  function saveExistXml() {
    ...
    $.ajax({
      url: "db_update_xml.php",
      type: "POST",
      data: {"xml":xmlText
            <?php if (isset($_GET['botId'])){
               echo",\"botId\":".$_GET['botId'];
            }?>},
      success: function(data) {
          alert(data);
          window.location.href = "/chatbot.php";
